...
class ModelBasicQA(dspy.Signature):
    """与用户进行聊天对话"""

    history = dspy.InputField(desc="与用户最近的聊天记录")
    msg = dspy.InputField(desc="用户消息")
    output = dspy.OutputField(desc="回应消息")
...
IntentType = Literal["chat_without_deep_thinking", "chat_with_deep_thinking"]


class Intent(dspy.Signature):
    """分析用户本次对话的意图"""

    msg = dspy.InputField(desc="用户的消息")
    output: IntentType = dspy.OutputField(desc="用户本次消息的意图")
...
class RelativeTime2AbsoluteTime(dspy.Signature):
    ...
    relative = dspy.InputField(
        desc="包含相对时间表述的句子(如昨天、今天、上个月、去年...)"
    )
    absolute = dspy.OutputField(
        desc="采用绝对时间表述的句子(如 2024年5月7日下午14点31分...)"
    )
