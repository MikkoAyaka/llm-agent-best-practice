...
from llm_agent_best_practice.signature.signatures import MemoryRecall, RelativeTime2AbsoluteTime, AgentChat
...
class BionicQA(dspy.Module):

    def __init__(self, agent):
        super().__init__()
        self.agent = agent
        self.mrag_retriever = MRAG(agent.agent_id)
        self.time_sentence_converter = dspy.ReAct(signature=RelativeTime2AbsoluteTime, tools=[realtime_tool_func])
    ...
    def forward(self, question):
        ...
        summarized_memory = dspy.Predict(signature=MemoryRecall)(related_memory=related_memory)
        return self.respond(memory=summarized_memory, history=self.agent.history[-4:], user_msg=question)
